...
public class ChildController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "All children found",
                    content = @Content(schema = @Schema(implementation = ChildResponseDTO.class))),
            @ApiResponse(responseCode = "404", description = "No children found", content = @Content)
    })
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Update an existing child")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Child updated",
                    content = @Content(schema = @Schema(implementation = ChildResponseDTO.class))),
            @ApiResponse(responseCode = "404", description = "Child not found", content = @Content)
    })
    public ResponseEntity<?> update(
        ...
        try {
            Child toUpdate = ChildMapper.fromDTO(dto);
            toUpdate.setId(id);
            Child updated = childService.save(toUpdate);
